...
OptionsBattlescapeState::OptionsBattlescapeState(OptionsOrigin origin) : OptionsBaseState(origin)
{
	...
	_navigableControls.push_back(_cbxEdgeScroll);
	_navigableControls.push_back(_cbxDragScroll);
	_navigableControls.push_back(_slrScrollSpeed);
	_navigableControls.push_back(_slrFireSpeed);
	_navigableControls.push_back(_slrXcomSpeed);
	_navigableControls.push_back(_slrAlienSpeed);
	_navigableControls.push_back(_btnArrows);
	_navigableControls.push_back(_btnTuCost);
	_navigableControls.push_back(_btnTooltips);
	_navigableControls.push_back(_btnDeaths);
...
void OptionsBattlescapeState::setFocusedControlVisuals(InteractiveSurface* control, bool focused)
{
    ...
    else if (sl) { /* Slider focus visual currently not explicit */ }
...
void OptionsBattlescapeState::handle(Action *action)
{
	...
	if (openComboBox) { 
		...
		SDLKey sym = action->getDetails()->key.keysym.sym;
		// Determine step for sliders. These ranges are mostly 1-20 or 1-40.
		int sliderStep = 1; 
		...
		if (sym == Options::keyMenuUp) {
			...
			if (_focusedControl) {
				...
				if (_focusedControl == _btnArrows) btnPathPreviewClick(&clickAction); // Both path preview buttons call this
